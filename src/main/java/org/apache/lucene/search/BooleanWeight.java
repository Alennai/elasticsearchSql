...
final class BooleanWeight  extends Weight {
            ...
            if (e.isMatch()) {
                if (c.isScoring()) {
                    ...
                    subs.add(Explanation.match(0f, "match on required clause, product of:",
                            Explanation.match(0f, BooleanClause.Occur.FILTER + " clause"), e));
    ...
    BulkScorer booleanScorer(LeafReaderContext context) throws IOException {
        final int numOptionalClauses = query.getClauses(BooleanClause.Occur.SHOULD).size();
        final int numRequiredClauses = query.getClauses(BooleanClause.Occur.MUST).size() + query.getClauses(BooleanClause.Occur.FILTER).size();
    ...
    public ScorerSupplier scorerSupplier(LeafReaderContext context) throws IOException {
        ...
        if (scorers.get(BooleanClause.Occur.SHOULD).size() == minShouldMatch) {
            // any optional clauses are in fact required
            scorers.get(BooleanClause.Occur.MUST).addAll(scorers.get(BooleanClause.Occur.SHOULD));
            scorers.get(BooleanClause.Occur.SHOULD).clear();
            minShouldMatch = 0;
        }

        if (scorers.get(BooleanClause.Occur.FILTER).isEmpty() && scorers.get(BooleanClause.Occur.MUST).isEmpty() && scorers.get(BooleanClause.Occur.SHOULD).isEmpty()) {
            // no required and optional clauses.
            return null;
        } else if (scorers.get(BooleanClause.Occur.SHOULD).size() < minShouldMatch) {
        ...
        if (!needsScores && minShouldMatch == 0 && scorers.get(BooleanClause.Occur.MUST).size() + scorers.get(BooleanClause.Occur.FILTER).size() > 0) {
            scorers.get(BooleanClause.Occur.SHOULD).clear();
        }
